...
func (n *Generator) start(ctx context.Context) {
	logx.L().Infof("start loop id generator, %v", time.Now())
	err := n.register(ctx)
	if err != nil {
...
func (n *Generator) register(ctx context.Context) (e error) {
	session, err := concurrency.NewSession(consensus.GetClient(), concurrency.WithTTL(20))
	if err != nil {
		logx.L().Errorf("register err %v", err)
		return err
	}
	getRes, err := session.Client().Get(ctx, confutil.GetConf().GetMaxNodeIDPrefix())
	if err != nil {
		logx.L().Errorf("register err %v", err)
		return err
	}
	...
	k := fmt.Sprintf("%s/%d", confutil.GetConf().GetNodeIDPrefix(), nodeID)
	addr := toolutil.GetIp()
	txRes, err := session.Client().Txn(ctx).
		...
		Then(v3.OpPut(k, fmt.Sprintf("%s", addr),
			...
			v3.OpPut(confutil.GetConf().GetMaxNodeIDPrefix(), strconv.FormatInt(nodeID, 10))).
		Commit()
	if err != nil {
		logx.L().Errorf("register err %v", err)
		return err
...
func InitIdGenerate(ctx context.Context) {
